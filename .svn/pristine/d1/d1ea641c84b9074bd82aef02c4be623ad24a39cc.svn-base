...
function initEncuesta() {
	...
	if (errorServicio == 0) {
		...
		setTimeout(function() {
			$("#modal-init").find(".header-senc").css("display", "none");
			$("#encuesta-init").fadeIn(250);
			$("#encuesta-init").css("display", "block");
			$("#encuesta-init").find(".header-senc").addClass(
					"animated slideInDown");
			$("#encuesta-init").find(".content-tabs").addClass(
					"animated slideInUp");
		}, 500);
	} else {
		mostrarNotificacion('warning', 'asd');
	}
...
function comenzarEncuesta() {
	$.ajax({
		...
		if (data.error == 0) {
			...
			actualizarObjJSON(JSON.parse(data.jsonObj));
	...
	if(tipoEncuGlobal == 'P' || tipoEncuGlobal == 'A') {
		...
		.done(function(data) {
			...
			if(data.error == 0) {
				...
				$('#divPregInicial').html(data.arraPregIni);
			} else if(data.error == 1) {
				mostrarNotificacion('error', data.msj, 'Error');
			}
		});
	} else {
		$('#divCombosPadreAlum').css("display", "none");
		$.ajax({
			url: "c_encuesta/getPreguntaInicial",
			data : { tipoEncuGlobal : tipoEncuGlobal },
	        async : false,
	        type: 'POST'
		})

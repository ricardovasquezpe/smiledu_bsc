	...
	Pace.track(function() {
		...
		$.ajax({
			...
			try{
				...
				$("#tbBusquedaAuxiliaresApoyoAdmSede").bootstrapTable({});
				console.log(data.tablaBusqueda);
				$(document).ready(function(){
	        	    $('[data-toggle="tooltip"]').tooltip(); 
	            });
				modal("modalConfirmAsignarApoyoAdmSede");
...
function abrirModalDeleteApoyoAdmSede(persona){
	personaApoyoAdmSede = persona;
	modal("modalConfirmDeleteApoyoAdmSede");
}
...
function eliminarApoyoAdmSede(){
	...
	Pace.track(function() {
		$.ajax({
			data : {persona : personaApoyoAdmSede},
			url  : 'c_detalle_evento/eliminarApoyoAdmSede',
			type : 'POST'
		}).done(function(data) {
			try{
				data = JSON.parse(data);

...
function abrirModalAgendarContacto(contacto, fecha, hora){
	cons_contacto = contacto;
	setearInput("fechaAgendarContacto", fecha);
    setearInput("horaAgendarContacto", hora);
		...
		if(fecha.length > 0 && hora.length > 0){
			$.ajax({
				data : {contacto : cons_contacto,
						fecha    : fecha,
						hora     : hora},
				url  : 'c_evaluar_rapido/validateCantCont',
				type : 'POST'
			}).done(function(data) {
				data = JSON.parse(data);
				$("#msjCantContactosAgendar").html(data.msj);
			});
		}
	});
}
	...
	Pace.track(function() {
		...
		.done(function(data) {
			try {
				...
				$("#tituloResumenDiag").html("Resumen Diagn\u00F3stico: "+nombreContact);
